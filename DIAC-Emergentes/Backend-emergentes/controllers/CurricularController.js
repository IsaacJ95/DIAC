...
const create = async (req, res, next) => {
    ...
    try {
        ...
        metodologiasArray.forEach(async metodologia => {
            const newMetodologia = await MetodologiaCurricular.create({description:metodologia})
        ...
        resourcesArray.forEach(async resource => {
            const newResource = await ResourceCurricular.create({description:resource})
        ...
        resultsArray.forEach(async result => {
            const newResult = await ResultFinalCurricular.create({description:result})
            await doc.addResultFinalCurricular(newResult)
        })

        //respuesta
        res.json({
            doc, docWithCourses, docWithTeacher, body, destrezasArray, objectivesArray, criteriasArray,
            metodologiasArray, resourcesArray, resultsArray
        })
